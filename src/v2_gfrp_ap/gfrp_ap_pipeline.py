...
def plotPixelIntensity(x,y):
    cap = cv.VideoCapture(os.path.join(DATA_DIR, '1010_3030_6059.mp4'))
    intensityValues = []
    ...
    while(True):
        ...
        if not ret:
            ...
            intensityValues.append(frame[y][x])
...
def displayDetrendedVideo(display_or_save):
    ...
    if display_or_save == '1':
        ...
        out = cv.VideoWriter('output_correct.mp4', fourcc, 25.0, (WIDTH, HEIGHT), isColor=False)
...
def displayFourierTransform(display_or_save):
    # cap = cv.VideoCapture('output_correct.mp4')
    # ret, frame = cap.read()
...
croppedCorrMatrix = CORRMATRIX[400:610, :, :]
...
def displayCroppedCorrelationMatrix(display_or_save):
    ...
    elif display_or_save == 'disc': 
        disc_levels = int(input("Enter the number of discretization levels: "))
        cdSize = 2*disc_levels - 1
        disc3Dmatrix = np.zeros((disc_levels, HEIGHT-1, WIDTH-1), dtype=np.float64)
        corrDiscMatrix = np.zeros((cdSize, HEIGHT-1, WIDTH-1), dtype=np.float64)
        
        
        #padding
        for y in range(HEIGHT-1):
            for x in range(WIDTH-1):
                intensityValues = DETRENDEDMATRIX[:, y, x]
                padded_intensityValues = np.pad(intensityValues, (0, max(0, disc_levels - len(intensityValues))), 'constant', constant_values=(0))
                disc3Dmatrix[:, y, x] = padded_intensityValues
                print(y, x)

        #correlating
        for y in range(HEIGHT-1):
            for x in range(WIDTH-1):
                corrDiscMatrix[:, y, x] = np.correlate(disc3Dmatrix[:, y, x], disc3Dmatrix[:, 200, 200], 'full')
                print(y, x)

        selection = int(input("save or plot? (1 or 2): "))
        if selection == 1:
            #save 
            corrDiscMatrix = cv.normalize(corrDiscMatrix, None, 0, 255, cv.NORM_MINMAX)
            corrDiscMatrix = corrDiscMatrix.astype(np.uint8)

            fourcc = cv.VideoWriter_fourcc(*'mp4v')
            out = cv.VideoWriter(os.path.join(DATA_DIR, '1010_3030_6059.mp4'), fourcc, 25.0, (WIDTH-1, HEIGHT-1), isColor=False)

            for i in range(cdSize):
                frame = corrDiscMatrix[i]
                out.write(frame)
...
def cropMainLobe():
    ...
    ret, frame = cap.read()
    fourcc = cv.VideoWriter_fourcc(*'mp4v')
    out = cv.VideoWriter(os.path.join(DATA_DIR, 'mainLobe_1010_2020_4039_disccorr.mp4'), fourcc, 25.0, (458, 458), isColor=False)
    video3Dmatrix = np.zeros((4039, 458, 458), dtype=np.float64)
    i = 0
    while ret:
        video3Dmatrix[i] = cv.cvtColor(frame, cv.COLOR_BGR2GRAY)
        ret, frame = cap.read()
    ...
    croppedVideoMatrix = video3Dmatrix[1910:2120, :, :]

    for i in range(210):
        out.write(croppedVideoMatrix[i])
